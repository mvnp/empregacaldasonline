...
interface Propriedades {
    candidatos: CandidatoAdmin[]
}

export default function AdminCandidatosClient({ candidatos }: Propriedades) {
    ...
    }, [candidatos, busca, filtroCargo, filtroCidade, filtroStatus])
    ...
    const cargosOpcoes = [...new Set(candidatos.map(c => c.cargo))].map(c => ({ value: c, label: c }))
    const cidadesOpcoes = [...new Set(candidatos.map(c => c.local))].map(c => ({ value: c, label: c }))
                ...
                {visiveis.map(c => (
                    <div key={c.id} style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.875rem' }}>
                            ...
                            <div style={{ minWidth: 0, paddingRight: 40 /* prevent overlap with status */ }}>
                                <h3 style={{ fontSize: '0.9rem', fontWeight: 700, color: '#09355F', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.nome}</h3>
                                <p style={{ fontSize: '0.78rem', color: '#2AB9C0', fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.cargo}</p>
                            </div>
                        </div>

                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', marginBottom: '0.875rem', minHeight: 60 }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem', marginBottom: '0.875rem', minHeight: 38 }}>
                            {c.habilidades && c.habilidades.slice(0,4).map(h => (
                            ...
                            {c.habilidades && c.habilidades.length > 4 && (
                                <span style={{ padding: '2px 8px', borderRadius: 9999, fontSize: '0.68rem', fontWeight: 600, background: '#f1f5f9', color: '#64748b' }}>
                                    +{c.habilidades.length - 4}
                                </span>
                            )}
                        </div>
