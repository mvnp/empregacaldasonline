...
import { cadastrarEmpresaAdmin, EmpresaFormData } from '@/actions/empresas'

function CadastrarEmpresaForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true)
        const res = await cadastrarEmpresaAdmin(form)
        setLoading(false)

        if (res.success) {
            window.location.href = `/admin/empresas/${res.id}`
        } else {
            setErro(res.error || 'Erro desconhecido ao cadastrar.')
        }
    }
    ...
    return (
        <div>
            {/* Header seguindo o modelo de /admin/candidatos/cadastrar */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
                ...
                <div>
                    <h1 style={{ fontSize: '1.5rem', fontWeight: 900, color: '#09355F' }}>Cadastrar Empresa</h1>
                    <p style={{ fontSize: '0.8rem', color: '#64748b' }}>Preencha os dados e crie o usuário empregador</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div style={cardStyle}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                        <div style={{ gridColumn: '1 / -1' }}>
                            <label style={labelStyle}>E-mail de Contato/Acesso *</label>
                            ...
                            <span style={{ fontSize: '0.75rem', color: '#64748b', marginTop: '0.4rem', display: 'block' }}>Este e-mail será usado para criar a conta de acesso do empregador.</span>
                        </div>
                        <div>
...
export default function CadastrarEmpresaPage() {
    return (
        <Suspense fallback={<div style={{ padding: '3rem', textAlign: 'center', color: '#94a3b8' }}>Carregando...</div>}>
