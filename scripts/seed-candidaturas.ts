...
async function seed() {
    ...
    if (erroCandidatos || !candidatos || candidatos.length === 0) {
        console.error('❌ Erro ou nenhum candidato encontrado:', erroCandidatos?.message)
        process.exit(1)
    }
    ...
    if (erroVagas || !vagas || vagas.length < 6) {
        console.error('❌ Erro ou quantidade insuficiente de vagas. É necessário pelo menos 6 vagas.', erroVagas?.message)
        process.exit(1)
    }
