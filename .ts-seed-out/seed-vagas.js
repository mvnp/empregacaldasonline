...
function normModalidade(m) {
    ...
    return (_a = map[m.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')]) !== null && _a !== void 0 ? _a : 'presencial';
}
/** Normaliza tipo_contrato para o CHECK do banco */
function normContrato(c) {
    var _a;
    const map = {
        clt: 'clt',
        pj: 'pj',
...
const VAGAS = [
    ...
    // ── 2: Analista de Marketing Digital ──────────────────────────────────────
    {
        ...
        destaque: false,
        descricao: 'Esta é uma excelente oportunidade para profissionais da área de Marketing que desejam crescer em uma agência sólida e inovadora.',
        responsabilidades: [
    ...
    // ── 3: Gerente Comercial ───────────────────────────────────────────────────
    {
        ...
        destaque: true,
        descricao: 'Esta é uma excelente oportunidade para profissionais da área de Vendas que desejam crescer em uma empresa sólida e inovadora.',
        responsabilidades: [
    ...
    // ── 4: Designer UX/UI ─────────────────────────────────────────────────────
    {
        ...
        destaque: false,
        descricao: 'Esta é uma excelente oportunidade para profissionais da área de Tecnologia que desejam crescer em uma empresa sólida e inovadora.',
        responsabilidades: [
    ...
    // ── 5: Enfermeiro(a) UTI ──────────────────────────────────────────────────
    {
        ...
        destaque: false,
        descricao: 'Esta é uma excelente oportunidade para profissionais da área de Saúde que desejam crescer em uma empresa sólida e inovadora.',
        responsabilidades: [
    ...
    // ── 6: Engenheiro Civil ───────────────────────────────────────────────────
    {
        ...
        destaque: false,
        descricao: 'Esta é uma excelente oportunidade para profissionais da área de Engenharia que desejam crescer em uma empresa sólida e inovadora.',
        responsabilidades: [
    ...
    // ── 7: Professor de Matemática ────────────────────────────────────────────
    {
        ...
        destaque: false,
        descricao: 'Esta é uma excelente oportunidade para profissionais da área de Educação que desejam crescer em uma empresa sólida e inovadora.',
        responsabilidades: [
...
async function seed() {
    ...
    for (const vaga of VAGAS) {
        ...
        const { data: vagaRow, error: vagaErr } = await supabase
            ...
            email_contato: (_a = vaga.email) !== null && _a !== void 0 ? _a : null,
            link_externo: (_b = vaga.link_externo) !== null && _b !== void 0 ? _b : null,
            status: 'ativa',
            ...
            .single();
        if (vagaErr || !vagaRow) {
            console.error(`❌  Erro ao inserir vaga "${vaga.titulo}":`, vagaErr === null || vagaErr === void 0 ? void 0 : vagaErr.message);
            continue;
        }
        ...
        if (vaga.responsabilidades.length > 0) {
            const { error } = await supabase.from('vaga_responsabilidades').insert(vaga.responsabilidades.map((texto, i) => ({ vaga_id: vagaId, texto, ordem: i })));
            if (error)
                console.error(`   ⚠️  responsabilidades:`, error.message);
        }
        // 3. Requisitos
        if (vaga.requisitos.length > 0) {
            const { error } = await supabase.from('vaga_requisitos').insert(vaga.requisitos.map((texto, i) => ({ vaga_id: vagaId, texto, ordem: i })));
            if (error)
                console.error(`   ⚠️  requisitos:`, error.message);
        }
        // 4. Diferenciais
        if (vaga.diferenciais.length > 0) {
            const { error } = await supabase.from('vaga_diferenciais').insert(vaga.diferenciais.map((texto, i) => ({ vaga_id: vagaId, texto, ordem: i })));
            if (error)
                console.error(`   ⚠️  diferenciais:`, error.message);
        }
        // 5. Benefícios
        if (vaga.beneficios.length > 0) {
            const { error } = await supabase.from('vaga_beneficios').insert(vaga.beneficios.map((texto, i) => ({ vaga_id: vagaId, texto, ordem: i })));
            if (error)
                console.error(`   ⚠️  beneficios:`, error.message);
        }
    }
    console.log('\n🎉  Seed concluído!\n');
}
