...
async function seed() {
    ...
    for (let i = 0; i < 80; i++) {
        ...
        if (candErr || !candRow) {
            console.error(`❌ Erro candidatos para ${email}:`, candErr === null || candErr === void 0 ? void 0 : candErr.message);
            continue;
        }
        ...
        for (let j = 0; j < resAll.length; j++) {
            if (resAll[j].error)
                console.error(`   ⚠️ erro em itens relacionais [${j}]: ${(_a = resAll[j].error) === null || _a === void 0 ? void 0 : _a.message}`);
        }
